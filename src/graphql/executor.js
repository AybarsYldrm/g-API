...
const { ensurePermission, permissionMaskFrom } = require('../utils/permissions');
...
function checkPermissions(fieldMeta, args, ctx) {
  ...
  if (typeof fieldMeta.permissionMask === 'number' && fieldMeta.permissionMask > 0) {
    ensurePermission(user, fieldMeta.permissionMask);
  }
}
    ...
    try {
      ...
      if (typeof fieldDef === 'function') {
        ...
        meta = {
          ...
          permissionMask: fieldDef.permissions !== undefined ? permissionMaskFrom(fieldDef.permissions) : 0
        };
      } else {
      ...
      data[key] = null;
      errors.push(formatGraphQLError(e, key));
    }
  }
  ...
  for (const sel of selectionSet.selections) {
    ...
    if (typeof resolver === 'function') actualResolver = resolver;
      ...
      meta = {
        ...
        permissionMask: resolver.permissions !== undefined ? permissionMaskFrom(resolver.permissions) : 0
      };
    }
...
function formatGraphQLError(err, fieldKey) {
  const extensions = Object.assign({}, err.extensions || {});
  if (err.statusCode && !extensions.httpStatus) {
    extensions.httpStatus = err.statusCode;
  }
  if (!extensions.code) {
    if (err.statusCode === 401) extensions.code = 'UNAUTHENTICATED';
    else if (err.statusCode === 403) extensions.code = 'FORBIDDEN';
    else extensions.code = 'INTERNAL_SERVER_ERROR';
  }
  return {
    message: err.message,
    path: [fieldKey],
